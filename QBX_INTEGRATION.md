--[[
    ...
    This file shows how to integrate the fertilizer items with QBX-Core (QBCore fork).
    Follow these steps to add the items to your QBX server.
]]

--[[
    Step 1: Add items to qbx_core shared items
...
--[[
    ...
    The script automatically detects QBX-Core and handles usage.
    However, you may want to add item callbacks for proper integration.
    
    Add to your resource that handles items (or qbx_core/server/main.lua):
]]
...
--[[
    ...
    If using qb-inventory or ox_inventory with QBX:
...
-- In qb-shops/config.lua or qbx-shops config
Config.Shops = {
    ['farming_shop'] = {
...
--[[
    Step 5: Using with ox_lib (QBX compatible)
...
--[[
    ...
    If you want to track fertilizer usage in database:
]]
...
--[[
    Step 7: Advanced integration with qbx-target or ox_target
]]
...
--[[
    Step 8: Inventory removal on use (automatic)
    
    The script automatically removes items from inventory when used:
    - Works with qb-inventory
    - Works with ox_inventory
    - Works with ps-inventory
    - Automatic detection
]]
...
--[[
    ...
    - QBX-Core is optimized fork of QBCore
